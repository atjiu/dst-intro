...
目前可拓印的建筑有:猪人火炬、猎犬冢、高脚鸟巢、空心树桩、猴子巢、龙虾窝、月光龙虾窝、尖壳蜗牛窝、远古窑。
...
进入下一生长阶段时，除正常按照植株需求计算压力值外，还会固定增加2点压力值。

注1：该动作无关罪恶值。

注2：每个植株的一生只能执行一次拔苗助长。

注3：压力值的增加意味着植株必定无法巨大。

注4：该动作优先级较低且为右键操作，仅手部无装备时可执行。
            `,
            recipe: "",
            tech: ""
        }, {
...
④若物品栏中有大理石，可消耗大理石\*1并额外生成一个墓碑，死者、修墓者和入土时间将刻在墓志铭上。理智回复buff持续时间翻倍;
...
| 对象 | 海象营地 |
|---|---|
| 产出 | 1/4概率成功。产物中贝雷帽权重为3，海象牙初始权重为1。海象牙权重与海象营地数量及海象再生速度呈负相关 |
| 险 | 若成功则出现三只蓝色猎犬围攻 |
| 相关CD | 1天后再洗劫 |
| 最大存量 | 1个 |
| 罪恶值 | 1 |
...
· 再P.S.默认海象再生速度下，4个海象营地将使海象牙权重为1，1个海象营地对应权重为3
            `,
            recipe: "",
            tech: ""
        }, {
...
食用后攻击力翻倍，效果持续半天。龙心灌蛋会被龙心覆盖（并触发后遗症）。
...
我来这儿之前从没吃过这个。
            ...
            name: "蛋白粉",
            image: "ndnr_albumenpowder.png",
            code: "ndnr_albumenpowder",
            recipe: "鸟腿\*4",
            fresh: "永久",
            foodtype: "GOODIES",
            description: `
...
腐烂后变为臭豆腐。
            ...
            name: "豆浆",
            image: "ndnr_soybeanmilk1.png",
            code: "ndnr_soybeanmilk1",
            recipe: "豆粉\*1+冰块\*2+甜度\*1",
            fresh: "6天",
            foodtype: "GOODIES",
            description: `
| 饱食 | 理智 | 生命 |
...
你觉得甜的好喝还是咸的好喝？
            ...
            name: "豆奶",
            image: "ndnr_soybeanmilk2.png",
            code: "ndnr_soybeanmilk2",
            recipe: "豆粉\*1+冰块\*1+甜度\*1+乳制品度\*1",
            fresh: "6天",
            foodtype: "GOODIES",
            description: `
| 饱食 | 理智 | 生命 |
...
可开凿。开凿后获得2~3个冰块。
            ...
            name: "臭豆腐",
            image: "ndnr_stinkytofu.png",
            code: "ndnr_stinkytofu",
            recipe: "",
            fresh: "20天",
            foodtype: "VEGGIE",
            description: `
...
收获大豆作物、破坏巨型大豆或将大豆给予鸟笼中的鸟可获得。

【种植】

可种植出大豆植株，适应季节为夏、秋，水分需求为1级（同土豆），肥料配比与洋葱互补。
            `,
            tech: ""
        }, {
            name: "大豆",
            image: "ndnr_soybean.png",
            code: "ndnr_soybean",
            fresh: "6天",
            foodtype: "VEGGIE",
            recipe: "",
...
可升级箱子：冰箱、盐盒、姐妹骨灰盒、箱子、龙鳞宝箱。

能量核存放在蘑菇灯、菌伞灯中也能使其发光，并使其他格子的物品保持新鲜度。
            `,
            tech: ""
        }, {
            name: "触手血",
...
在非水面上使用暗影心房对坎普斯背包进行实验，将其改造为暗影生命——堕落虫洞，成功概率1/3(旺达、麦斯威尔必定成功)。

堕落虫洞被删除时（地基崩坏或指令）会返还一个坎普斯背包。
...
成对生成的堕落虫洞会自动连接起来，传送效果原版虫洞，跳过虫洞的玩家会损失15点理智和5点生命。

【移除】
1. 夜间将化石碎片给予堕落虫洞，堕落虫洞有1/16的概率变成暗影编织者（不同区域形态不同），并返还坎普斯背包；失败不消耗化石碎片
2. 将魔力泉水给予堕落虫洞，会净化暗影心房的力量，使堕落虫洞变回坎普斯背包；魔力泉水会被消耗
            `,
            tech: ""
        }, {
            name: "蟹钳",
...
若获取过程中任意暗影BOSS死于过量伤害（一击必杀类），将最终不会掉落战利品。

【容器】

装备在背部（优先）或身体部位。拥有14格空间，与坎普斯背包相同。
...
原版和本模组的部分海鲜料理食用时会增加痛风值，痛风值每天会降低7点。

入夜时若痛风值不少于100点会触发痛风，移动速度-50%。

食用酸奶（低脂)可立即降低10点痛风值。注: wx-78和沃特不会痛风。
...
友好果蝇喜甜食，给予友好果蝇（清醒时）一个蜂王浆，它会感到非常幸福，照料农田的效率大大增加（可一次照顾一块耕地）。效果持续3天。
            `,
            tech: ""
        }, {
            name: "火鸡",
...
①自然再生：与多肉植物相同，在夏季绿洲湖泊有水时，会生成在湖泊周围（上限为12颗）
...
2. 可作祟复活，代价同重生护符；作祟CD为一天，作祟时一个屏幕范围内的魔力花都会进入作祟CD；魔力花被种下时也会进入作祟CD
...
50%几率掉落怪物肉，2.5%几率掉落蛇油，5%几率掉落毒腺，10%几率掉落蛇皮。
...
在黄昏/夜晚活跃，白天非战斗状态将返回曼德拉丘，会主动攻击玩家（植物人除外）。
...
保暖值60，防水值70%，具有绝缘效果。满耐久8天，每次修补恢复5天耐久。

穿上后蛇会视你为友好，且青蛙不敢主动攻击你。
...
防水值70%，具有绝缘效果。满耐久8天，每次修补恢复5天耐久。

戴上后蛇会视你为友好，且青蛙不敢主动攻击你。
            `,
            tech: "衣物、雨具"
        }, {
            name: "蛇皮地毯",
...
树种子只限于松果、桦栗果、多枝树种、棕榈松果树芽、椰子，其余种子均无效。
...
有类似烹饪锅的四格物品槽，只有冶炼配方中的物品可以被放进去，是冶炼科技的起点。

冶炼失败（配方不正确）将消耗所有材料，并返还灰烬\*1。

可批量炼制，多余物品自动掉落，冶炼时间与产物数量成正比。
            `,
            tech: "蓝图-建筑、科技"
        }, {
            name: "远古伪科学基地",
...
居住着一位曼德拉长老，曼德拉长老死后一天会生成新的曼德拉长老。（月圆的时候还会对你笑！）
            ...
            image: "高脚鸟巢.png",
            code: "tallbirdnest",
            recipe: "小鸟*1+草*10+树枝*10",
            description: `
【介绍】

对高脚鸟巢拓印获得制作蓝图

在泥泞地皮或岩石地皮上建造一个高脚鸟巢，和野外一样。
            `,
            tech: "蓝图-建筑"
        }, {
            name: "空心树桩",
...
在落叶林地皮上建造一个空心树桩，和野外一样。

附注：杜绝猫窝再生的一个有效方法是，摧毁猫窝后将原地皮覆盖为人工地皮。原理见穆白white的视频BV1Z94y1S73z
            `,
            tech: "蓝图-建筑"
        }, {
            name: "猪人火炬",
...
建造一个尖壳蜗牛窝，和洞穴的一样。
            ...
            name: "远古窑",
            image: "ndnr_archive_cookpot.png",
            code: "archive_cookpot",
            recipe: "铥矿\*1+木炭\*6+树枝\*6",
            description: `
【介绍】
...
注：本模组对远古窑进行了修改，其烹饪时间-20%（速度与便携烹饪锅相同）
            `,
            tech: "蓝图-建筑"
        }, {
            name: "洒水器",
            image: "ndnr_sprinkler.png",
            code: "ndnr_sprinker",
            recipe: "合金\*1+蓝宝石\*6+冰块\*6",
            description: `
【介绍】

放置在距离岸边不超过5个地皮的位置，会自动寻找最近的水面延伸水管。水管经过的地皮无法耕种。

工作状态下会持续洒水，洒水范围2.5个地皮，每秒产生5个水滴。水滴落地时可为耕地增加5点潮湿，还会阻止闷烧、浇灭明火。
...
腐乳粉末可以使被调味的料理的饱食度回复效果变为1.5倍，精神回复效果变为0.75倍。料理増加前缀：（臭的）
            ...
            name: "豆粉",
            image: "ndnr_soybeanmeal.png",
            code: "ndnr_soybeanmeal",
            recipe: "大豆*3",
            description: `
【介绍】

（仅沃利）可制作，一次制作获得两个。不可用于调味。
...
可烹饪，相关料理：豆浆、豆奶。
            ...
            name: "根箱",
            image: "ndnr_treasurechest_root.png",
            code: "ndnr_treasurechest_root",
            recipe: "食人花种子*1+绿宝石*1+木板*3",
            description: `
【制作】

与植物亲近者可以制作（拥有"plantkin"的tag，如沃姆伍德）。

【功能】

12格空间，当前世界里所有根箱共用一个容器。
            `,
            tech: "暗影操作器-角色"
        }]
    }, {
...
梦魇尖喙和爬行梦魇获得查理的恩赐后，除了都拥有更魁梧的体态外有着不同的改变。

爬行梦魇化身的查理卫士移速与威尔逊持平（6），拥有3000点生命和50点伤害，发动攻击的间隔-40%；

梦魇尖喙化身的查理卫士移速高达14点，拥有1500点生命和100点伤害，发动攻击的间隔-33%；
...
当她忍无可忍时，就会让拥有数倍于普通暗影生物力量的查理卫士前来绞杀。

【机制】

玩家执行的本模组动作均会改变自己的罪恶值（初始为0，不同动作的罪恶值见下面描述）。

执行任意改变罪恶值的动作后，若罪恶值为10/30/60/100便会进行一次判定，判定成功的概率分别为0.01/0.09/0.36/1.00。

执行任意改变罪恶值的动作后，若罪恶值大于100，必定触发查理卫士来袭。


若判定成功，查理卫士将会从玩家周围3~5个地皮的距离内随机位置生成的裂隙中出现并仇恨触发判定的玩家，同时清空罪恶值。
...
总结：最好的奖励为彩虹宝石，最差的情况（非保底）是2个大理石。
            `
        }]
    }
];
